  ...
  options = {
    hyprnix.xdg = {
      enable = mkOption {
        type = types.bool;
        default = true;
        description = "Enable the hyprnix XDG MIME application configuration.";
      };
      mimeAppDefaults = mkOption {
        type = types.attrsOf (types.listOf types.str);
        default = {};
        description = ''
          Additional or overriding MIME application mappings for XDG.
          These are merged into the default `xdg.mimeApps.defaultApplications` values.
        '';
      };
    };
  };

  config = mkIf config.hyprnix.xdg.enable {
    xdg.mimeApps = {
      enable = true;
      defaultApplications = lib.recursiveUpdate {
      ...
      } config.hyprnix.xdg.mimeAppDefaults;
    };
  };
}
