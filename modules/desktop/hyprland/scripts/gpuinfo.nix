...
pkgs.writeShellApplication {
  ...
  runtimeInputs = with pkgs; [
    ...
    gnused         # provides sed
    jq             # provides jq
    # amd-smi / nvidia-smi are usually provided by system drivers
  ];
  ...
  text = ''
    ...
    detect() { # Auto detect Gpu used by Hyprland(declared using env = AQ_DRM_DEVICES)
    ...
    general_query() { 
      ...
      temperature=\$(echo "\${sensors_data}" | \${filter} grep -m 1 -E "(edge|Package id.*|junction|hotspot)" | awk -F ':' '{print int(\$2)}') 
      ...
      if [ -n "\$temperature" ] && [ "\$temperature" != "N/A" ]; then
        for card in /sys/class/drm/card*/device; do
          if [ -L "\$card/driver" ] && basename "\$(readlink -f "\$card/driver")" 2>/dev/null | grep -qi "amdgpu"; then
            [ -z "\$utilization" ] && [ -f "\${card}/gpu_busy_percent" ] && utilization=\$(cat "\text{\${card}}/gpu_busy_percent" 2>/dev/null)
