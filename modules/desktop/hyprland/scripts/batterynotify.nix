...
pkgs.writeShellApplication {
  ...
  runtimeInputs = with pkgs; [
    ...
    upower         # provides upower CLI tools
    dbus           # provides dbus-monitor
    systemd        # ensures systemctl availability for fallbacks
  ];
  ...
  text = ''
    ...
    while (( "\$#" )); do  
      case "\$1" in
        ...
        *|"--help"|"-h")
          ...
          echo "Visit https://github.com/prasanthrangan/hyprdots for the Github Repo"
          exit 0
          ;;
      esac
    done
        ...
        case "\$battery_status" in         
            ...
            *)
                if [[ ! -f "/tmp/hyprdots.batterynotify.status.fallback.\$battery_status-\$\$" ]]; then 
                    echo "Status: '==>> \$battery_status <<==' Script on Fallback mode. Unknown power status."
    ...
    EOF
            ...
            if \$verbose; then 
                current_pid=\$\$
                pids=\$(pgrep -f "batterynotify.sh" || echo "")
                for pid in \$pids ; do 
                    if [ "\$pid" -ne "\$current_pid" ] ; then 
                        kill -STOP "\$pid" 
            ...
            # Read early values to satisfy structural conditionals
            for battery in /sys/class/power_supply/BAT*; do  
            ...
            dbus-monitor --system "type='signal',interface='org.freedesktop.DBus.Properties',path='\$(upower -e | grep battery | head -n1)'" 2> /dev/null | while read -r battery_status_change; do 
                fn_status_change  
            done
        fi
    }
