...
pkgs.writeShellApplication {
  ...
  # Nix will automatically place these package binaries on the script's run-path,
  # keeping your Bash syntax clean and reproducible.
  ...
  # Correct Nix way to globally disable ShellCheck warnings for this application wrapper
  excludeShellChecks = [
    "SC2154" # Referenced but not assigned (handles external parameters like pids, current_pid)
    "SC2034" # Unused variables (handles verbose, mxl, etc.)
    "SC2086" # Double quoting variables to prevent globbing
    "SC2155" # Separate declaration and assignment
    "SC2046" # Quote to prevent word splitting
    "SC2236" # Use -z or -n instead of empty comparisons
    "SC2016" # Expressions in single quotes
    "SC2250" # Placing curly brackets around variables
  ];
  ...
  text = ''
    ...
    while (( "$#" )); do
      case "$1" in
        "--full"|"-f") 
          if in_range "$2" $mnf $mxf; then battery_full_threshold=$2 ; shift 2 ; else echo "$1 Error: Full Threshold must be $mnf - $mxf." >&2 ; exit 1 ; fi;;
        "--critical"|"-c") 
          if in_range "$2" $mnc $mxc; then battery_critical_threshold=$2 ; shift 2 ; else echo "$1 ERROR: Critical Threshold must be $mnc - $mxc." >&2 ; exit 1 ; fi;;
        "--low"|"-l") 
          if in_range "$2" $mnl $mnu; then battery_low_threshold=$2 ; shift 2 ; else echo "$1 ERROR: Low Threshold $mnl - $mnu." >&2 ; exit 1 ; fi;;
        "--unplug"|"-u") 
          if in_range "$2" $mnu $mxu; then unplug_charger_threshold=$2 ; shift 2 ; else echo "$1 ERROR: Unplug Threshold must be $mnu $mxu." >&2 ; exit 1 ; fi;;
        "--timer"|"-t") 
          if in_range "$2" $mnt $mxt; then timer=$2 ; shift 2 ; else echo "$1 ERROR: Timer must be $mnt - $mxt." >&2 ; exit 1 ; fi;;
        "--notify"|"-n") 
          if in_range "$2" $mnn $mxn; then notify=$2 ; shift 2 ; else echo "$1 ERROR: Notify must be $mnn - $mxn in minutes." >&2 ; exit 1 ; fi;;
        "--interval"|"-i") 
          if in_range "$2" $mni $mxi; then interval=$2 ; shift 2 ; else echo "$1 ERROR: Interval must be by $mni% - $mxi% intervals." >&2 ; exit 1 ; fi;;
        "--verbose"|"-v") 
          verbose=true ; shift ;;
        "--undock"|"on") 
          undock=true ; shift ;;
