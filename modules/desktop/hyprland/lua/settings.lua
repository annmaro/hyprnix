...
hl.on("hyprland.start", function()
    ...
    -- --- SAFE CLIPHIST CLEANUP ---
    -- Fetch the variable via Lua rather than trusting raw shell expansion
    local cache_dir = os.getenv("XDG_CACHE_HOME")
    local home_dir = os.getenv("HOME")

    -- Fallback to standard spec (~/.cache) if XDG_CACHE_HOME is empty
    ...
    -- Only execute the rm command if we successfully verified a safe, explicit path
    if cache_dir and cache_dir ~= "" and cache_dir ~= "/" then
        hl.exec_cmd("rm -f '" .. cache_dir .. "/cliphist/db'")
    end
end)
